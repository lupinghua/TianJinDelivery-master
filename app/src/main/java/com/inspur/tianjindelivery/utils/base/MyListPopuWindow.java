package com.inspur.tianjindelivery.utils.base;
...
import com.inspur.tianjindelivery.adapter.DialogScreenAdapter;
...
public  class MyListPopuWindow {
    ...
    public static void showTopDialog(Context context, final Activity activity,View view,int viewId, List<ScreenDialogBean> screenStatusList,OnItemClickListener monItemClickListener){
        ...
        initContactsBottmoPopuStatusTop(mContext);
        if (mcontactsBottompopupStatus.isShowing()) {
            mcontactsBottompopupStatus.dismiss();// 关闭
        } else {
            mcontactsBottompopupStatus.setWidth(ViewGroup.LayoutParams.WRAP_CONTENT);
            mcontactsBottompopupStatus.setHeight(ViewGroup.LayoutParams.WRAP_CONTENT);
            if(view == null) {
                mcontactsBottompopupStatus.showAsDropDown(activity.findViewById(viewId));
            }else{
    ...
    private static void initContactsBottmoPopuStatusTop(Context context) {
        ...
        mcontactsBottompopupStatus
                .setAnimationStyle(R.style.contactPopuAnimationExitTop);
        mcontactsBottompopupStatus.setOutsideTouchable(true);


        //项目状态初始化
        dialogScreenAdapterStatus = new DialogScreenAdapter(mContext,R.layout.item_line,mScreenStatusList,false);
